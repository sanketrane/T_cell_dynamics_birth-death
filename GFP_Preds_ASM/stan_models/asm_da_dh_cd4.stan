functions{
  ...
  real delta_age(real age, real[] parms){
    real del0   = parms[2];
    real r_del = parms[4];
    real q_h   = parms[5];
    real time = parms[6];
  ...
  // rate of net loss depending on cell and host age
  real lambda_age(real age, real[] parms){

    real value  = delta_age(age, time, parms) - rho_age(age, parms);  //  changes with cell and host age
    return value;
  }

  // rate of net loss depending on cell and host age
  real alpha_age(real age, real[] parms){

    real value  = delta_age(age, time, parms) + rho_age(age, parms);  //  changes with cell and host age
    return value;
  }

  real[] lambda_integrand(real age,  real[] y, real[] parms, real[] x_r,  int[] x_i) {
    real value = lambda_age(age, parms);
    return {value};
  }

  real[] alpha_integrand(real age,  real[] y, real[] parms, real[] x_r,  int[] x_i) {
    real value = alpha_age(age, parms);
    return {value};
  }

  real lambda_int(real[] parms) {
    int x_i[0];
    real value = integrate_ode_rk45(lambda_integrand, {0.0}, 0.0, rep_array(parms[6], 1), parms, {0.0}, x_i)[1, 1];
    return value;
  }

  real alpha_int(real[] parms) {
    int x_i[0];
    real value = integrate_ode_rk45(alpha_integrand, {0.0}, 0.0, rep_array(parms[6], 1), parms, {0.0}, x_i)[1, 1];
    return value;
  }
  ...
  real lambda_integ(real lo_lim, real up_lim, real[] parms, real time){
    ...
    real del_host = del0 * (4*del0/(1+(time/q_h)^5)); // delta of cell age = 0 varies with time.

    real value = ((del_host/r_del) * (exp(-r_del * lo_lim) - exp(-r_del * up_lim))) - rho * (up_lim - lo_lim);
    return value;
  }

  // function that calculates intgral of net process rate --  solved analytically to speed up the numerical integration
  real alpha_integ(real lo_lim, real up_lim, real[] parms, real time){
    real del0  = parms[2];
    real rho   = parms[3];
    real r_del = parms[4];
    real q_h   = parms[5];
  ...
  real Asm_init_age(real age, real time, real[] parms) {
    ...
    real value = g_age(t0, parms) * exp(- lambda_integ(age - time + t0, age, parms, time - age));
    return value;
   }
   ...
   real Asm_theta_age(real age, real time, real[] parms) {
     ...
     real value =  theta_spline(time - age, parms) * exp(- lambda_integ(0, age, parms, time - age));
     return value;
    }
  ...
  real Asm_total_age(real age, real time, real[] parms){
    ...
    if(age < (time - t0)) {
      value =  theta_spline(time - age, parms) * exp(- lambda_integ(0, age, parms, time - age));
      } else {
        value = g_age(t0, parms) * exp(- lambda_integ(age - time + t0, age, parms, time - age));
    }
  ...
  real U_init_ki_age(real ki, real age, real time, real[] parms){
    ...
    if (ki <= exp(-beta * (time - t0)) ){
      value = g_age(t0, parms) * ki_dist_init(ki * exp(beta * (time - t0))) * exp(beta * (time - t0)) * exp(- alpha_integ(age - time + t0, age, parms, time - age));
    } else {
      value = 2.0 * rho_age(age, parms, time - age) * Asm_init_age(age - tau, time - tau, parms) * (1/(beta * ki)) * exp(- alpha_integ(age - tau, age, parms, time - age));
    }
    return value;
  }
  ...
  real U_theta_ki_age(real ki, real age, real time, real[] parms){
    ...
    if (ki <= exp(-beta * age)) {
      value = theta_spline(time - age, parms) * ki_dist_theta(ki * exp(beta * age), time - age, parms) * exp(beta * age) * exp(- alpha_integ(0.0, age, parms, time - age));
    } else {
      value = 2.0 * rho_age(age, parms, time - age) * Asm_theta_age(age - tau, time - tau, parms) * (1/(beta * ki)) * exp(- alpha_integ(age - tau, age, parms, time - age));
    }
    return value;
  }
