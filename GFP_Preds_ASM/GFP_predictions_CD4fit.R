...
GFPposKineg_df <- readxl::read_excel(file.path(DataDir, "RagGFP_ontogeny_pooled2.xlsx"), sheet = 3)%>%
  select('mouseID', 'location', 'age', 'TH.SP4', "SP.4nai", 'LN.4nai') 
...
GFP_pos_total <- full_join(GFPposKineg_df, GFPposKipos_df, 
  ...
  mutate(gfp_pos_Thy = TH.SP4.x + TH.SP4.y,
         gfp_pos_LN = LN.4nai.x + LN.4nai.y,
         gfp_pos_Spl = SP.4nai.x + SP.4nai.y)%>%
...
p0 <- ggplot(GFP_pos_total)+
  ...
  labs(x="Host age", y=NULL, title="GFP % total cd4 T cells") + ylim(0, 100)
...
p1 <- ggplot()+
  geom_line(aes(x=Timeseq, y=GFP_frac_fit*100), col=2)+
...
p3 <- ggplot(GFP_ki_split)+
  ...
  labs(x="Host age", y=NULL, title="GFP % in cd4 T cells") + 
  #scale_x_log10() +
  #scale_y_log10()+
...
ggplot()+
  ...
  labs(x="Host age", y=NULL, title="Predictions of GFP % in CD4 T cells") + 
  geom_line(data=GFP_split_pred_df, aes(x=Host_age, y=GFP_frac*100), col=2)+
  facet_wrap(.~ subset, labeller = grid_labels) +
  scale_x_log10() +myTheme #+
  #scale_color_discrete(name=NULL,labels=c('LN', "Spleen")) 



#ggsave(filename = file.path(OutputDir, "gfp_fit.pdf"), p1, device = "pdf", width = 3.5, height = 3)
...
GFP_numbers_df <- readxl::read_excel(file.path(DataDir, "RagGFP_ontogeny_pooled2.xlsx"), sheet = 1)%>%
  ...
  mutate(age.at.S1K = age,
         total_counts = LN.4nai + SP.4nai) %>%
  select('mouseID', contains('S1K'), contains('counts'))

GFP_data <- full_join(GFPnegKipos_df, GFPposKipos_df, by = c('mouseID', 'location', 'age'))%>%
  mutate(Ki_Thy = (TH.SP4.x + TH.SP4.y)/100,
         Ki_LN= (LN.4nai.x + LN.4nai.y)/100,
         Ki_Spl = (SP.4nai.x + SP.4nai.y)/100) %>%
...
counts_ln  <- Onto_counts %>%
  filter(organ == "LN") %>%
  select(mouse.id, age.at.S1K.days, CD4.nai)%>%
...
ggplot()+
  labs(x="Host age", y=NULL, title="Ki67 % in CD4 T cells") + 
...
ggplot()+
  labs(x="Host age", y=NULL, title="Total counts of CD4 T cells") + 
  scale_x_log10()+ 
  scale_color_manual(name=NULL, values=c(2,"#0571c4","#037971"))+
...
##Host age effects
hostageDir <- file.path(WorkinDir, "stan_models", paste0("asm_da_dh_cd4.stan"))
rstan::expose_stan_functions(hostageDir)

params_HA <- c(params_imported, 25)
Timeseq_HA<- seq(from = 10,
                 to = 300,
                 length.out=200)



delta_time <- function(Time, parms){
  del_0 <- parms[2]
  q_h <- parms[5]
  
  del_0 + (4*del_0/(1+(Time/q_h)^5))
}
ggplot()+
  geom_line(aes(x=Timeseq, y=delta_time(Timeseq, params_HA)))+
  labs(x="Host age", y=NULL, title="Loss rate of RTE (delta0)") + 
  scale_x_log10()

ggsave(filename = file.path(OutputDir, "loss_rate_RTE.pdf"), last_plot(), device = "pdf", width = 5.5, height = 4.5)



Kipos_HA_frac <- ki67_pos_frac(Timeseq_HA, params_HA)
...
ggplot()+
  ...
  geom_line(aes(x=Timeseq_HA, y=Kipos_HA_frac*100), col='magenta') #+ myTheme

ggsave(filename = file.path(OutputDir, "Ki67pred_Hostage.pdf"), last_plot(), device = "pdf", width = 6.5, height = 4.5)


GFPpos_kipos_HA_preds <- GFPpos_frac(Timeseq_HA, par_estm) * Kipos_HA_frac
GFPpos_kineg_HA_preds <- GFPpos_frac(Timeseq_HA, par_estm) * (1 - Kipos_HA_frac)

GFP_HA_pred_df <- data.frame("Host_age" = Timeseq_HA,
...
ggplot()+
  ...
  geom_line(data=GFP_split_pred_df, aes(x=Host_age, y=GFP_frac*100), col=2)+
  geom_line(data=GFP_HA_pred_df, aes(x=Host_age, y=GFP_frac*100), col="magenta")+
  facet_wrap(.~ Subset, labeller = grid_labels) +
  scale_x_log10() +
...
ggsave(filename = file.path(OutputDir, "gfppreds_Hostage.pdf"), last_plot(), device = "pdf", width = 12, height = 4.5)
