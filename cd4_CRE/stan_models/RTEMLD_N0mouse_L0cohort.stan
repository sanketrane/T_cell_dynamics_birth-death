functions{
  ...
  real[] shm_ont(real t, real[] y, real[] parms, real[] rdata,  int[] idata) {
    ...
    // ki hi RTE
    dydt[1] = rho_rte * (1 - mu) * (2 * y[2] + y[1]) - (beta + delta_rte + (rho_rte * mu)) * y[1];
    // ki lo RTE
    dydt[2] = beta * y[1] - (rho_rte + delta_rte) * y[2];
    // ki hi mN
...
transformed parameters{
  ...
  for (i in 1:N){
    parms[1] = lambda[group_id[i]];          // random effects in lambda0
