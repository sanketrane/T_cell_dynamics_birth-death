...
model{
  ...
  mu_r ~ normal(0.01, 0.1);
  sigma_r ~ normal(0.001, 0.001);

  //model definition
  N0 ~ normal(mu_N0, sigma_N0);
  r_lambda ~ normal(mu_r, sigma_r);
...
generated quantities{
  ...
  for (i in 1:numPred){
    // group 1
    parms1[1] = N0[1];   parms1[2] = lambda;  parms1[3] = mean(r_lambda[1:12]);   parms1[4] = q_hostage;  parms1[5] = day_stamp[1];  parms1[6] = t0_group[1];
    y1_mean_pred[i] = N_total_time(tseq1[i], parms1);

    // group 2
    parms2[1] = N0[2];   parms2[2] = lambda;  parms2[3] = mean(r_lambda[13:29]);  parms2[4] = q_hostage;  parms2[5] = day_stamp[2];  parms2[6] = t0_group[2];
    y2_mean_pred[i] = N_total_time(tseq2[i], parms2);

    // group 3
    parms3[1] = N0[3];   parms3[2] = lambda;  parms3[3] = mean(r_lambda[30:41]);  parms3[4] = q_hostage;  parms3[5] = day_stamp[3];  parms3[6] = t0_group[3];
    y3_mean_pred[i] = N_total_time(tseq3[i], parms3);

    // group 4
    parms4[1] = N0[4];   parms4[2] = lambda;  parms4[3] = mean(r_lambda[42:53]);  parms4[4] = q_hostage;  parms4[5] = day_stamp[4];  parms4[6] = t0_group[4];
    y4_mean_pred[i] = N_total_time(tseq4[i], parms4);

    // group 5
    parms5[1] = N0[5];   parms5[2] = lambda;  parms5[3] = mean(r_lambda[54:66]);  parms5[4] = q_hostage;  parms5[5] = day_stamp[5];  parms5[6] = t0_group[5];
