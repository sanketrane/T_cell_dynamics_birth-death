functions{
  ...
  real g_age_dist(real age, real[] parms, real[] rdata){
    real value;
    real N0 = parms[1];
    real day_stmp = 1;     // day of tamoxifen treatment for each group
    real t0 = 14;           // first tipme point of observation for each group
  ...
  real N_total(real[] parms){
    ...
    real value = integrate_ode_rk45(G_age_time, {0.0}, time - day_stmp - 6, rep_array(time - day_stmp, 1), parms, {0.0}, x_i)[1, 1];
    return value;
  }
...
model{
  ...
  r_lambda ~ normal(0.01, 0.2);
  sigma_counts ~ normal(0, 2);

  //hyper priors
  //mu_N0 ~ normal(5.6, 1);
  //sigma_N0 ~ normal(0, 0.5);
