...
parameters{
  ...
  real lambda[J, K];           // vector of rate of loss for each animal
...
transformed parameters{
  ...
  for (i in 1:N){
    parms[1] = N0[group_id[i]];              // random effects in N0 -- group level
    parms[2] = lambda[group_id[i], mouse_id[i]];          // random effects in lambda0 -- mouse level
...
generated quantities{
  ...
  for (i in 1:numPred){
    // group 1
    parms1[1] = N0[1];   parms1[2] = mean(lambda[1:12]);  parms1[3] = r_lambda;  parms1[4] = day_stamp[1];  parms1[5] = t0_group[1];
    y1_mean_pred[i] = N_total_time(tseq1[i], parms1);

    // group 2
    parms2[1] = N0[2];   parms2[2] = mean(lambda[13:29]);  parms2[3] = r_lambda;  parms2[4] = day_stamp[2];  parms2[5] = t0_group[2];
    y2_mean_pred[i] = N_total_time(tseq2[i], parms2);

    // group 3
    parms3[1] = N0[3];   parms3[2] = mean(lambda[30:41]);  parms3[3] = r_lambda;  parms3[4] = day_stamp[3];  parms3[5] = t0_group[3];
    y3_mean_pred[i] = N_total_time(tseq3[i], parms3);

    // group 4
    parms4[1] = N0[4];   parms4[2] = mean(lambda[42:53]);  parms4[3] = r_lambda;  parms4[4] = day_stamp[4];  parms4[5] = t0_group[4];
    y4_mean_pred[i] = N_total_time(tseq4[i], parms4);

    // group 5
    parms5[1] = N0[5];   parms5[2] = mean(lambda[54:66]);  parms5[3] = r_lambda;  parms5[4] = day_stamp[5];  parms5[5] = t0_group[5];
