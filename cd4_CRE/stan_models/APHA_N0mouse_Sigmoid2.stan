functions{
  ...
  real g_age_dist(real age, real[] parms){
    real value;
    real N0 = parms[1];
    real day_stamp = parms[6];     // day of tamoxifen treatment for each group
    real t0 = parms[7];           // first tipme point of observation for each group
  ...
  real lambda_integ(real lo_lim, real up_lim, real[] parms, real time){
    ...
    // lambda0 varying with host age
    real lambda0_hostage = lambda_hostage(time, parms);

    // lambda varying with host and cell age
    real value = (lambda0_hostage/r_lambda) * (exp(-r_lambda * lo_lim) - exp(-r_lambda * up_lim));
    return value;
  }
  ...
  real[] G_age_time(real age, real[] y, real[] parms, real[] rdata, int[] idata){
    ...
    if (age > time - t0){
      value = g_age_dist(age - time + t0, parms) * exp(- lambda_integ(age - time + t0, age, parms, time  - age));
    } else {
      value = 0.0;
    }
