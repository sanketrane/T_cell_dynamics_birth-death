...
ggplot()+
  ...
  scale_y_log10(limits = c(5e5, 3e7)) 
...
G_age_time <- function(age, Time, dstmp, t0, N0, l0, r_l, r_h){
  l0_hostage = l0 * exp(-r_h * t0)
  g_age_vec(age - Time + t0, dstmp, t0, N0) *
           exp(- integrate(lambda_age, lower=age - Time + t0, upper=age, l0=l0_hostage, r_l=r_l)$value)
}
...
n_total_time <- function(Time, dstmp, t0, N0, l0, r_l, r_h){
    integrate(G_age_vec, lower = Time - dstmp - 5, upper = Time - dstmp, Time=Time,
            dstmp=dstmp, t0=t0, N0=N0, l0=l0, r_l=r_l, r_h=r_h)$value
}
...
library(nlme)
datasetG <- groupedData(naive_labelled ~ 1|id, naive_df)
nlin.mix <- nlme(log(naive_labelled) ~ log(n_total_vec(Time=age_anim, dstmp=age_group, t0=t0_group,
                                              N0=N0_group, l0=l0, r_l=r_l, r_h=r_h)), data=datasetG, 
                 fixed = list(l0 ~ 1, r_l ~ 1, r_h ~1),
                 random = r_l + r_h ~ 1|id,
                 start = c(l0=0.05, r_l=0.01, r_h = 0.005))
...
sim1 <- n_total_vec(Time=tseq1, dstmp=1, t0=14, N0=N0_list$N0_g1, l0=parms[1], r_l=parms[2], r_h=parms[3])
tseq2 <- seq(20, 120, length.out = 50)
sim2 <- n_total_vec(Time=tseq2, dstmp=7, t0=28, N0=N0_list$N0_g2, l0=parms[1], r_l=parms[2], r_h=parms[3])
tseq3 <- seq(35, 105, length.out = 50)
sim3 <- n_total_vec(Time=tseq3, dstmp=28, t0=42, N0=N0_list$N0_g3, l0=parms[1], r_l=parms[2], r_h=parms[3])
tseq4 <- seq(60, 160, length.out = 50)
sim4 <- n_total_vec(Time=tseq4, dstmp=56, t0=70, N0=N0_list$N0_g4, l0=parms[1], r_l=parms[2], r_h=parms[3])
tseq5 <- seq(180, 280, length.out = 50)
sim5 <- n_total_vec(Time=tseq5, dstmp=175, t0=189, N0=N0_list$N0_g5, l0=parms[1], r_l=parms[2], r_h=parms[3])
...
ggsave(filename = 'asmNhostage_a5dist_flat_p1.pdf', width = 8, height = 5, units = "in")
...
ggsave(filename = 'asmNhostage_a5dist_flat_p2.pdf', width = 6, height = 4.5, units = "in")
