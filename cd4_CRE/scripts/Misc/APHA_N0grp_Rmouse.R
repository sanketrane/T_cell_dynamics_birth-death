...
init_list <- function(){
  list(
    mu_r = exp(rnorm(1, log(0.01), 0.1)),
    mu_N0 = rnorm(1, 5.5, 0.1),
    lambda = exp(rnorm(1, log(0.05), 0.1)),
    q_hostage   = exp(rnorm(1, log(0.001), 0.1)),
    sigma_counts = runif(1, 0.1, 0.3),
    sigma_N0   = runif(1, 0, 0.5),
    sigma_r    = runif(1, 0, 0.01)
  )
}
