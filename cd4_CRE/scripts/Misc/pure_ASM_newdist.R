...
g_age <- function(age, dstmp, t0, N0, p0){
  tb = t0 - dstmp
  ta = t0 - dstmp - 12
  b0 = 10^N0 * p0/(exp(tb*p0) - exp(ta*p0))
  if(age < t0 - dstmp - 12){
   value = 0.0
...
aseq <- seq(0, 14, 0.1)
plot(aseq, g_age_vec(aseq, 1, 14, 5.3, 0.1))
integrate(g_age_vec, lower = 0, upper = 14, dstmp=1, t0=14, N0=6, p0=0.1)
...
## total cell age distribution at time == Time
G_age_time <- function(age, Time, dstmp, t0, N0, l0, r_l, p0){
  g_age_vec(age - Time + t0, dstmp, t0, N0, p0) *
           exp(- integrate(lambda_age, lower=age - Time + t0, upper=age, l0=l0, r_l=r_l)$value)
}
## vectorizing the function
G_age_vec <- Vectorize(G_age_time)

## total counts of labeled cells at time == Time
n_total_time <- function(Time, dstmp, t0, N0, l0, r_l, p0){
    integrate(G_age_vec, lower = Time - dstmp - 12, upper = Time - dstmp, Time=Time,
            dstmp=dstmp, t0=t0, N0=N0, l0=l0, r_l=r_l, p0=p0)$value
}
## vectorizing the function
...
LL_naive_labelled <- function(param, boot_data) { 
  ...
  p0  <- param[3]             #parameters to be estimated as part of a vector
  
  k  <- length(param)          #number of unknown parameters 
  n1 <- nrow(boot_data)        #number of observations in the data-set
  
  fit_df <- boot_data %>%
    mutate(naive_fit = n_total_vec(Time=age_anim, dstmp=age_group, t0=t0_group, N0=N0_group,
                                   l0=l0, r_l=r_l, p0=p0))
...
fit_naive_labelled <- optim(par=c(0.05, 0.001, 0.5), fn=LL_naive_labelled, boot_data=naive_df,
                            control = list(trace = TRUE, maxit = 1000))
fit_naive_labelled
aiccd8
parms <- fit_naive_labelled$par
parms = c(0.05, 0.01, 0.1)

## predictions for individual age cohorts
tseq1 <- seq(10, 120, length.out = 50)
sim1 <- n_total_vec(Time=tseq1, dstmp=1, t0=14, N0=N0_list$N0_g1, l0=parms[1], r_l=parms[2], p=parms[2])
tseq2 <- seq(20, 120, length.out = 50)
sim2 <- n_total_vec(Time=tseq2, dstmp=7, t0=28, N0=N0_list$N0_g2, l0=parms[1], r_l=parms[2], p=parms[2])
tseq3 <- seq(35, 105, length.out = 50)
sim3 <- n_total_vec(Time=tseq3, dstmp=28, t0=42, N0=N0_list$N0_g3, l0=parms[1], r_l=parms[2], p=parms[2])
tseq4 <- seq(60, 160, length.out = 50)
sim4 <- n_total_vec(Time=tseq4, dstmp=56, t0=70, N0=N0_list$N0_g4, l0=parms[1], r_l=parms[2], p=parms[2])
tseq5 <- seq(180, 280, length.out = 50)
sim5 <- n_total_vec(Time=tseq5, dstmp=175, t0=189, N0=N0_list$N0_g5, l0=parms[1], r_l=parms[2], p=parms[2])
...
ggsave(filename = 'asm_a12dist_oldhigh_p1.pdf', width = 8, height = 5, units = "in")
...
ggsave(filename = 'asm_a12dist_oldhigh_p2.pdf', width = 6, height = 4.5, units = "in")
