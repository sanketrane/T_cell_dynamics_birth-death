...
n_total_time <- function(Time, dstmp, t0, N0, l0, r_l){
  ...
  integrate(G_age_vec, lower = Time - dstmp - 5, upper = Time - dstmp, Time=Time,
            dstmp=dstmp, t0=t0, N0=N0, l0=l0_j, r_l=r_l)$value
}

## vectoring function
n_total_vec <- Vectorize(n_total_time)

n_total_multilevel <- function(Time,dstmp, t0, N0, l0, r_l, beta_j){
  
  l0_j = l0 + beta_j
  
  n_total_vec(Time,dstmp, t0, N0, l0_j, r_l)
}


n_total_multilevel(1, 20, 1, 14, 6, 0.05, 0.001, )
