...
g_age <- function(age, dstmp, t0, N0, p0){
  tb = t0 - dstmp
  ta = t0 - dstmp - 5
  b0 = 10^N0 * p0/(exp(-ta*p0) - exp(-tb*p0))
  if(age < t0 - dstmp - 5){
   value = 0.0
...
integrate(g_age_vec, lower = 0, upper = 14, dstmp=1, t0=14, N0=5.3, p0=0.1)
10^5.3
...
fit_naive_labelled <- optim(par=c(0.05, 0.01, 0.1), fn=LL_naive_labelled, boot_data=naive_df,
...
ggsave(filename = 'plots/asm_a5dist_yghigh_p1.pdf', width = 8, height = 5, units = "in")
...
ggsave(filename = 'plots/asm_a5dist_yghigh_p2.pdf', width = 6, height = 4.5, units = "in")
