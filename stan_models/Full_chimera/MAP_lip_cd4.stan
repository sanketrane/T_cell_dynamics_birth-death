functions{
  ...
  real[] shm_ont(real t, real[] y, real[] parms, real[] rdata,  int[] idata) {
    ...
    real delta     = parms[2];
    real rho       = parms[3];
    real count_bar = parms[4];
    //real r_div     = parms[4];

    real dydt[2];
    real beta  = 1/3.5;            //rate of loss of ki67 -- mature naive cells

    real  rho_lip = rho / (1 + ((y[1] + y[2]) / count_bar)^3);
    //real rho_lip = rho * (1 + exp(-r_div * (y[1] + y[2] + y[3])));

    // ki hi mN
    dydt[1] = theta_spline(t, psi) * eps_spline(t) + rho_lip * (2 * y[2] +  y[1]) - (beta + delta) * y[1];
    // ki lo mN
    dydt[2] = theta_spline(t, psi) * (1 - eps_spline(t)) + beta * y[1] - (rho_lip  + delta) * y[2];
  ...
  real[] shm_chi(real time, real[] y, real[] parms, real[] rdata,  int[] idata) {
    ...
    real  rho_lip = rho / (1 + ((y[1] + y[2] + y[3] + y[4]) / count_bar)^3);
    // age of BMT in each recipient
    real ageAtBMT = parms[5];

    // ki hi donor
    dydt[1] = theta_spline(time, psi) * Chi_spline(time - ageAtBMT) * eps_spline(time) + rho_lip * (2 * y[2] +  y[1]) - (beta + delta) * y[1];
    // ki lo donor
    dydt[2] = theta_spline(time, psi) * Chi_spline(time - ageAtBMT) * (1 - eps_spline(time)) + beta * y[1] - (rho_lip  + delta) * y[2];

    // ki hi host
    dydt[3] = theta_spline(time, psi) * (1-Chi_spline(time - ageAtBMT)) * eps_spline(time) + rho_lip * (2 * y[4] +  y[3]) - (beta + delta) * y[3];
    // ki lo host
    dydt[4] = theta_spline(time, psi) * (1-Chi_spline(time - ageAtBMT)) * (1 - eps_spline(time)) + beta * y[3] - (rho_lip  + delta) * y[4];
  ...
  vector math_reduce(vector global_params, vector local_params, real[] x_r, int[] x_i){
    ...
    //params
    real N0 = global_params[5];
    real kappa0 = global_params[6];
    real init_cond[2];
    ...
    real ont_counts_mean;
    real ont_kiprop_mean;
    ...
    // deciding whether the unique data point is either from ontogeny or chimera dataset!
    if (dat_t0 == 1){
      // ODE solution for ontogeny dataset --  x_r = data time
      ont_solve = solve_ont(dat_time, init_cond, to_array_1d(global_params));
      ont_counts_mean = ont_solve[1] + ont_solve[2];
      ont_kiprop_mean = ont_solve[1]/ont_counts_mean;
      y_mean_stacked[1] = ont_counts_mean;
      y_mean_stacked[2] = ont_kiprop_mean;
      y_mean_stacked[3] = 0;
      y_mean_stacked[4] = 0;
    } else {
...
data{
  int<lower = 1> numOnt;
  real<lower = 0> ont_counts[numOnt];
  real<lower = 0> ont_ki[numOnt];
  int<lower = 1> numChi;
...
parameters{
  ...
  real<lower=0.0, upper=1.0> psi;
  real<lower=0.2, upper=1.0> kappa0;
  real count_bar;                  // density thresold at which rho halves

  real<lower=0> sigma_ont_counts;
  real<lower=0> sigma_ont_ki;
...
transformed parameters{
  ...
  vector[numOnt] y1_mean;               // PDE prediction for counts from ontogeny data
  vector[numOnt] y2_mean;               // PDE prediction for ki prop from ontogeny data
  ...
  vector[4*numOnt+4*numChi] y_mean_stacked;        // compliled output across all nodes

  global_params[1] = psi;
  global_params[2] = delta;
  global_params[3] = rho;
  global_params[4] = count_bar;
  global_params[5] = N0;
  global_params[6] = kappa0;
  ...
  for (i in 1:numChi){
    y3_mean[i] = y_mean_stacked[(4*numOnt)+ 4*i - 3];
    y4_mean[i] = y_mean_stacked[(4*numOnt)+ 4*i - 2];
    y5_mean[i] = y_mean_stacked[(4*numOnt)+ 4*i - 1];
    y6_mean[i] = y_mean_stacked[(4*numOnt)+ 4*i];
  }
}
...
model{
  ...
  sigma_ont_counts ~ normal(0, 2);
  sigma_ont_ki ~ normal(0, 2);
  sigma_chi_counts ~ normal(0, 2);
  sigma_Nfd ~ normal(0, 2);
  sigma_donor_ki ~ normal(0, 2);
  sigma_host_ki ~ normal(0, 2);

  log(ont_counts) ~ normal(log(y1_mean), sigma_ont_counts);
  asinsqrt_array(ont_ki) ~ normal(asinsqrt_array(to_array_1d(y2_mean)), sigma_ont_ki);

  log(chi_counts) ~ normal(log(y3_mean), sigma_chi_counts);
  asinsqrt_array(N_donor_fraction) ~ normal(asinsqrt_array(to_array_1d(y4_mean)), sigma_Nfd);
  asinsqrt_array(donor_ki) ~ normal(asinsqrt_array(to_array_1d(y5_mean)), sigma_donor_ki);
  asinsqrt_array(host_ki) ~ normal(asinsqrt_array(to_array_1d(y6_mean)), sigma_host_ki);
}
...
generated quantities{
  ...
  real ontcounts_pred[numPred];   real ontki_pred[numPred];
  ...
  // log likelihoods
  vector[numOnt] log_lik_ont_counts;
  vector[numOnt] log_lik_ont_ki;
  ...
  for (i in 1:numPred){
    y1_mean_pred[i] = y_ont_pred[i, 1] + y_ont_pred[i, 2];
    y2_mean_pred[i] = y_ont_pred[i, 1] / y1_mean_pred[i];

    ontcounts_pred[i] = exp(normal_rng(log(y1_mean_pred[i]), sigma_ont_counts));
    ontki_pred[i] = asinsqrt_inv(normal_rng(asinsqrt_real(y2_mean_pred[i]), sigma_ont_ki));
    ...
    y5_mean_pred1[i] = y_chi_pred1[i, 1]/(y_chi_pred1[i, 1] + y_chi_pred1[i, 2]);
    y5_mean_pred2[i] = y_chi_pred2[i, 1]/(y_chi_pred2[i, 1] + y_chi_pred2[i, 2]);
    y5_mean_pred3[i] = y_chi_pred3[i, 1]/(y_chi_pred3[i, 1] + y_chi_pred3[i, 2]);

    y6_mean_pred1[i] = y_chi_pred1[i, 3]/(y_chi_pred1[i, 3] + y_chi_pred1[i, 4]);
    y6_mean_pred2[i] = y_chi_pred2[i, 3]/(y_chi_pred2[i, 3] + y_chi_pred2[i, 4]);
    y6_mean_pred3[i] = y_chi_pred3[i, 3]/(y_chi_pred3[i, 3] + y_chi_pred3[i, 4]);
  ...
  for (i in 1:numOnt) {
    log_lik_ont_counts[i] = normal_lpdf(log(ont_counts[i]) | log(y1_mean[i]), sigma_ont_counts);
    log_lik_ont_ki[i]     = normal_lpdf(asinsqrt_real(ont_ki[i]) | asinsqrt_real(y2_mean[i]), sigma_ont_ki);
  }

  // calculating log likelihoods
  for (i in 1:numChi) {
    log_lik_chi_counts[i] = normal_lpdf(log(chi_counts[i]) | log(y3_mean[i]), sigma_chi_counts);
    log_lik_Nfd[i]        = normal_lpdf(asinsqrt_real(N_donor_fraction[i]) | asinsqrt_real(y4_mean[i]), sigma_Nfd);
    log_lik_donor_ki[i]   = normal_lpdf(asinsqrt_real(donor_ki[i]) | asinsqrt_real(y5_mean[i]), sigma_donor_ki);
    log_lik_host_ki[i]    = normal_lpdf(asinsqrt_real(host_ki[i]) | asinsqrt_real(y6_mean[i]), sigma_host_ki);
  }
}
