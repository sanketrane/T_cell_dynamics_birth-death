functions{
  ...
  real[] shm_ont(real t, real[] y, real[] parms, real[] rdata, int[] idata) {
    ...
    real delta  = parms[2];
    real rho    = parms[3];
    real size_dens = parms[4];

    real dydt[2];
    real beta  = 1/3.5;            //rate of loss of ki67 -- mature naive cells

    real delta_dd = delta * (1 + (y[1] + y[2]) / size_dens);     // density dependence -- turnover rate\

    // ki hi mN
    dydt[1] = theta_spline(t, psi) * eps_spline(t) + rho * (2 * y[2] +  y[1]) - (beta + delta_dd) * y[1];
    // ki lo mN
    dydt[2] = theta_spline(t, psi) * (1 - eps_spline(t)) + beta * y[1] - (rho  + delta_dd) * y[2];
  ...
  real[] shm_chi(real time, real[] y, real[] parms, real[] rdata,  int[] idata) {
    ...
    real delta_dd = delta * (1 + (y[1] + y[2] + y[3] + y[4]) / size_dens);     // density dependence -- turnover rate\

    // ki hi donor
    dydt[1] = theta_spline(time, psi) * Chi_spline(time - ageAtBMT) * eps_spline(time) + rho * (2 * y[2] +  y[1]) - (beta + delta_dd) * y[1];
    // ki lo donor
    dydt[2] = theta_spline(time, psi) * Chi_spline(time - ageAtBMT) * (1 - eps_spline(time)) + beta * y[1] - (rho  + delta_dd) * y[2];

    // ki hi host
    dydt[3] = theta_spline(time, psi) * (1-Chi_spline(time - ageAtBMT)) * eps_spline(time) + rho * (2 * y[4] +  y[3]) - (beta + delta_dd) * y[3];
    // ki lo host
    dydt[4] = theta_spline(time, psi) * (1-Chi_spline(time - ageAtBMT)) * (1 - eps_spline(time)) + beta * y[3] - (rho  + delta_dd) * y[4];
