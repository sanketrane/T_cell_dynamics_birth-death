functions{
  ...
  real theta_spline(real time, real[] parms){
    ...
    if (time <=72){
      ...
      value = psi * theta72 * exp(-s1*(time-72));
    }
    return value;
  }
...
transformed parameters{
  ...
  vector[(2*numOnt)] y_mean_stacked;        // compliled output across all nodes
  ...
  for (i in 1:numOnt){
    y1_mean[i] = y_mean_stacked[2*i - 1];
    y2_mean[i] = y_mean_stacked[2*i ];
  }
}
