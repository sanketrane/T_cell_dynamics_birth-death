...
y1_0 <- par_df[8] * par_df[1] ### kappa0 * N0
y2_0 <- (1 - par_df[8]) * par_df[1] ### (1 - kappa0) * N0
y3_0 <- 0 ### mature Naive
y4_0 <- 0### mature Naive
parstan_vec <- c(par_df[7], par_df[3], par_df[2], par_df[4], par_df[5], par_df[6]) 

## init conditions at t0 = 5
init_cond_ont <- c(y1_0, y2_0, y3_0, y4_0)
...
stan_pred_df1 <- data.frame("age.at.S1K" = ts_pred_chi1,
  ...
  mutate(total_counts = y_pred.1 + y_pred.2 + y_pred.3 + y_pred.4 + y_pred.5 + y_pred.6 + y_pred.7 + y_pred.8,
         Nfd = (y_pred.1 + y_pred.2 + y_pred.3 + y_pred.4)/(total_counts * chivec1),
         donor_ki = (y_pred.1 + y_pred.2)/(y_pred.1 + y_pred.2 + y_pred.3 + y_pred.4),
         host_ki = (y_pred.5 + y_pred.6)/(y_pred.5 + y_pred.6 + y_pred.7 + y_pred.8),
...
stan_pred_df2 <- data.frame("age.at.S1K" = ts_pred_chi2,
  ...
  mutate(total_counts = y_pred.1 + y_pred.2 + y_pred.3 + y_pred.4 + y_pred.5 + y_pred.6 + y_pred.7 + y_pred.8,
         ...
         donor_ki = (y_pred.1 + y_pred.2)/(y_pred.1 + y_pred.2 + y_pred.3 + y_pred.4),
         host_ki = (y_pred.5 + y_pred.6)/(y_pred.5 + y_pred.6 + y_pred.7 + y_pred.8),
         ageBMT_bin = rep('ageBMT_group2', length(ts_pred_chi1)))
...
stan_pred_df3 <- data.frame("age.at.S1K" = ts_pred_chi3,
  ...
  mutate(total_counts = y_pred.1 + y_pred.2 + y_pred.3 + y_pred.4 + y_pred.5 + y_pred.6 + y_pred.7 + y_pred.8,
         ...
         donor_ki = (y_pred.1 + y_pred.2)/(y_pred.1 + y_pred.2 + y_pred.3 + y_pred.4),
         host_ki = (y_pred.5 + y_pred.6)/(y_pred.5 + y_pred.6 + y_pred.7 + y_pred.8),
         ageBMT_bin = rep('ageBMT_group3', length(ts_pred_chi1)))
...
ki_ratio_df  <- data.frame("ageBMT_group1" = stan_pred_df1$donor_ki/stan_pred_df1$host_ki,
                           "ageBMT_group2" = stan_pred_df2$donor_ki/stan_pred_df2$host_ki,
                           "ageBMT_group3" = stan_pred_df3$donor_ki/stan_pred_df3$host_ki) %>%
  gather(key = ageBMT_bin, value = ki_ratio)  %>%
  bind_cols(age.at.S1K = c(ts_pred_chi1, ts_pred_chi2, ts_pred_chi3))

ggplot() +
  geom_hline(yintercept = 1.0, col = 'darkred', linetype = 2, size=1.3) +
  geom_line(data = ki_ratio_df, aes(x = age.at.S1K, y = ki_ratio, col = ageBMT_bin), size =1.25) + 
  geom_point(data = chimera_data, aes(x = age.at.S1K, y = ki_dh_ratio, col = ageBMT_bin), size =2.5) + 
  labs(x = 'Host age', y = NULL, title = paste0("Ratio of Ki67+ cells donor:host")) +
  scale_y_continuous(limits = c(0.1, 30), trans = 'log10', breaks = c(0.3, 1, 3, 10, 30)) + 
  #scale_x_log10(limits = c(50, 450), breaks = c(10, 30, 100, 300, 450)) +
  myTheme + guides(col=F) +
  facet_grid(. ~ ageBMT_bin, labeller = as_labeller(ageBMT_names)) +
  myTheme + theme(legend.position = c(0.95, 0.9)) 

pdf(file = file.path(SaveDir, paste0(ModelName, "_", Population, "_Pred%03d.pdf")),
...
parms_vec <- c('psi' = par_df[7], 'delta' = par_df, 'rho' = 0.001937711)
tBMT = 58
