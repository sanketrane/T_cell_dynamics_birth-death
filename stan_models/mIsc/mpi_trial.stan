functions{
  ...
  vector lp_reduce(vector global_params, vector local_params, real[] x_r, int[] x_i){
    ...
    real Y1[n] = x_r[1:n];                 // counts data
    real Y2[n] = x_r[(n+1):(2*n)];         // ki67 proportions data
    real y1_mean[n];                       //counts data for each shard
    real y2_mean[n];                       //ki67 data for each shard
    real y_hat[n, 2];
    ...
    lp = normal_lpdf(Y1 | to_vector(log(y1_mean)), sigma_counts) +
        normal_lpdf(Y2 | to_vector(logit(y2_mean)), sigma_ki);
