functions{
  ...
  vector lp_reduce(vector global_params, vector local_params, real[] x_r, int[] x_i){
    ...
    real ki_counts[n];                           // ki67 couhts predictions from the model
    ...
    y1_mean[1:number_uniques] = N_total_time(to_array_1d(to_vector(time_uniques[1:number_uniques])/1.0), to_array_1d(global_params));
