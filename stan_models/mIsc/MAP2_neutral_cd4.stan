functions{
 //spline 1
  ...
  vector map_tester(vector global_params, vector local_params, real[] x_r, int[] x_i){
    ...
    int data_time[n] = x_i[1:n];           // time -- host age
    real y_hat[n, 2];
    real y_mean_stacked[2 * n];
    real init_cond[2];
    real N0 = global_params[4];
    real kappa0 = global_params[5];
    real parms[3] = to_array_1d(global_params[1:3]);
    ...
    for (i in 1:(n)){
        y_mean_stacked[i] = y_hat[time_indices[i], 1] + y_hat[time_indices[i], 2];
        y_mean_stacked[i+n] = y_hat[time_indices[i], 1]/ y_mean_stacked[i];
      }
...
transformed parameters{
  ...
  map_val = map_tester(global_params, to_vector({0}), counts, data_time);
  print(map_val);
