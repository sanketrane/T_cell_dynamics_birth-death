functions{
  ...
  real[] shm(real t, real[] y, real[] parms, real[] rdata, int[] idata) {
    ...
    real beta_prime  = 1/3.5;      //rate of loss of ki67 -- VRTE cells
  ...
  vector math_reduce(vector global_params, vector local_params, real[] x_r, int[] x_i){
   ...
   for (i in 1:n){
     y1_mean[i] = y_hat[time_indices[i], 1] + y_hat[time_indices[i], 2] + y_hat[time_indices[i], 3] + y_hat[time_indices[i], 4];
     y2_mean[i] = (y_hat[time_indices[i], 1] + y_hat[time_indices[i], 3])/y1_mean[i];
   }
...
generated quantities{
  ...
  for (i in 1:numPred){
    ...
    y2_mean_pred[i] = (y_hat_pred[i, 1] + y_hat_pred[i, 3]) / y1_mean_pred[i];
