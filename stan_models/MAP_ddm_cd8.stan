functions{
  ...
  real[] foreach_ode(real ts, real t0, real[] init_cond, real[] parms) {
   ...
   real[,] solve_ode(real[] solve_time, real[] init_cond, real[] parms){
     ...
     for (i in 1:num_solve){
          if (solve_time[i] > 5.0){
            y_hat[i] = foreach_ode(solve_time[i], 5.0, init_cond, parms);
          } else {
            y_hat[i] = init_cond;
          }
        }
  ...
  vector math_reduce(vector global_params, vector local_params, real[] x_r, int[] x_i){
    ...
    real y_hat[n, 2];                      // ode solution
    vector[2*n] y_mean_stacked;                      // ode solution
    ...
    y_hat[1:number_uniques] = solve_ode(to_array_1d(to_vector(time_uniques[1:number_uniques])/1.0),
                              init_cond, to_array_1d(global_params));
...
parameters {
  real<lower=1E4, upper=1E6> N0;                  // total cells counts at t0
...
transformed parameters{
  ...
  vector[numObs] y1_mean;               // PDE prediction for counts
  vector[numObs] y2_mean;               // PDE prediction for ki prop
  vector[numObs] y_mean_stacked;
  ...
  for (s in 1:n_shards){
    y1_mean[1 + (s-1) * M:s * M] = y_mean_stacked[1 + (s-1) * 2*M : M + (s-1) * 2*M];
    y2_mean[1 + (s-1) * M:s * M] = y_mean_stacked[(M+1) + (s-1) * 2*M : s * 2*M];
  }
}
...
generated quantities{
  ...
  for (i in 1:numPred){
    ...
    deltadd_pred[i] = delta/(1 + y1_mean_pred[i]);
  }
