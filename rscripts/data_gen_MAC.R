...
ggplot() +
  geom_point(data= data_fit, aes(x=age.at.S1K, y=total_counts, col=dataset)) +
  scale_y_log10(limits = c(1e5, 4e7))

ggplot()+
  geom_point(data = chimera_data, aes(x=age.at.S1K, y=Nfd, col =ageBMT_bin)) +
  ylim(0,1.15) #+ scale_x_log10() 

ggplot()+
  geom_point(data = data_fit, aes(x=age.at.S1K, y=total_kiprop * 100, col = dataset))+
  scale_y_log10(limits = c(0.3, 100))



rstan::expose_stan_functions("stan_models/neutral_cd4.stan")
rstan::expose_stan_functions("stan_models/Full_chimera/MAP_asm_deltavar_cd8.stan")
