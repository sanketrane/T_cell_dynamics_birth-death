...
kidonor_df <- data.frame("ageBMT_group1" = Y6pred_bin1$median,
                         "ageBMT_group2" = Y6pred_bin2$median,
                         "ageBMT_group3" = Y6pred_bin3$median) %>%
...
kidonor_lb <- data.frame("ageBMT_group1" = Y6pred_bin1$lb,
                         "ageBMT_group2" = Y6pred_bin2$lb,
                         "ageBMT_group3" = Y6pred_bin3$lb) %>%
  gather(key = ageBMTbin, value = donor_ki) 
...
kidonor_ub <- data.frame("ageBMT_group1" = Y6pred_bin1$ub,
                         "ageBMT_group2" = Y6pred_bin2$ub,
                         "ageBMT_group3" = Y6pred_bin3$ub) %>%
  gather(key = ageBMTbin, value = donor_ki) 
...
kipred_ratio_D2H <- data.frame(kidonor_df, kihost_df)  %>%
  bind_cols(age.at.S1K = c(ts_pred_chi1, ts_pred_chi2, ts_pred_chi3)) %>%
  mutate(donor_host_ratio = donor_ki/host_ki) %>%
  select(-"ageBMTbin.1")
