...
## time points for solving the PDE                    # timepoints in observations 
data_time_chi <- chimera_data$age.at.S1K 
...
dat_t0 <- tb_chi
dat_time <- data_time_chi
numObs <- length(dat_time)

for (n_shards in c(numObs)) {
  stan_rdump(c("numChi", "chi_counts",  "N_donor_fraction", "donor_ki", "host_ki",
...
GFPposKipos_df <- readxl::read_excel(file.path("datafiles", "RagGFP_ontogeny_pooled.xlsx"), sheet = 6)%>%
  select('mouseID', 'age', 'LN.8nai')
GFPnegKipos_df <- readxl::read_excel(file.path("datafiles", "RagGFP_ontogeny_pooled.xlsx"), sheet = 5)%>%
  select('mouseID', 'age','LN.8nai')

GFPnumbers_df <- readxl::read_excel(file.path("datafiles", "RagGFP_ontogeny_pooled.xlsx"), sheet = 1)%>%
  select('mouseID', 'location', 'age', 'LN.8nai')
...
ModelDir <- file.path("stan_models/only_chimera", paste0("MAP_", ModelName, "_", Population, ".stan"))
OutputDir <- file.path("out_fit/only_chimera", Population, ModelName)
...
ts_pred_chi1 <- seq(66, 1450, length.out = 300)
##
