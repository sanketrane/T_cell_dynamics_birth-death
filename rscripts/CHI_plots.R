...
ModelName <- "asm_deltavar_cd4_K50"#args[1]
Population <- "cd4"#args[2]
hexcode <- args[3]

OutputDir <- file.path("output_csv", "only_chimera")
SaveDir <- file.path('out_fit', "only_chimera", Population, ModelName) 
...
model_fit <- read_stan_csv(file.path(OutputDir, paste0(ModelName, ".csv"))) 
## parameters to plot
num_pars <- which(model_fit@model_pars == 'sigma_Nfd')
...
combined_loglik <- cbind( loo_loglik3, loo_loglik4, loo_loglik5,loo_loglik6)#, loo_loglik1, loo_loglik2)
...
source(file.path("rscripts/CHI_plot_tools.R"))

pdf(file = file.path(SaveDir, paste0(Population, '_', ModelName, "ChiFig%03d.pdf")),
    width = 12, height = 9, onefile = FALSE, useDingbats = FALSE )

counts_facet <- ggplot() +
  geom_line(data = Y3pred_bin1, aes(x = timeseries, y = median), size =1, col = 2) +
  ...
  geom_ribbon(data = Y3pred_bin1, aes(x = timeseries, ymin = lb, ymax = ub), fill = 2, alpha = 0.2) +
  ...
  geom_point(data = chimera_data, aes(x = age.at.S1K, y = total_counts, col=ageBMT_bin), size=2, alpha = 0.95) +
...
Nfd_plot <- ggplot() +
  geom_hline(yintercept = 1.0, col = "#01848a" , linetype = 2, size=1.3) +
